...
public static class DbSeeder
{
    ...
    {
        // Ensure baseline team exists
        var team = context.Teams.FirstOrDefault(t => t.TeamName == "Alpha Squad");
        if (team is null)
        {
            team = new Team
            {
        ...
        if (!context.Sprints.Any(s => s.TeamId == team.TeamId))
        {
        ...
        if (!context.TeamSettings.Any(ts => ts.TeamId == team.TeamId))
        {
            context.TeamSettings.AddRange(settings);
            context.SaveChanges();
        }

        EnsureAdditionalTeams(context);
    }

    private static void EnsureAdditionalTeams(SprintMonitorDbContext context)
    {
        var teamSeed = new List<(string TeamName, string Description)>
        {
            ("Beta Builders", "Backend and API delivery team"),
            ("Gamma Mobile", "Cross-platform app and UX implementation team"),
            ("Delta DataOps", "Data pipelines and analytics enablement team"),
            ("Omega Platform", "Platform reliability and infrastructure team")
        };

        var createdAny = false;
        foreach (var seed in teamSeed)
        {
            var existing = context.Teams.FirstOrDefault(t => t.TeamName == seed.TeamName);
